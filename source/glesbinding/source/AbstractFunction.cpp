...
GLESBINDING_THREAD_LOCAL auto t_pos = -1;
...
void AbstractFunction::provideState(const int pos)
{
    ...
    // if a state at pos exists, it is assumed to be neglested before
    if (s_maxpos >= pos)
        return;
...
void AbstractFunction::neglestState(const int pos)
{
    assert(pos <= s_maxpos);
    assert(pos > -1);
