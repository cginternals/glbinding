...
#include <glscbinding/ContextInfo.h>
...
{
    const auto extension = glscbinding::Meta::getExtension(extensionName);
...
{
    const auto requiredExtensions = glscbinding::Meta::extensions(version);
    const auto requiredFunctions = glscbinding::Meta::functions(version);
